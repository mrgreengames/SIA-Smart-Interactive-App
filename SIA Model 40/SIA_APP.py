...
#     prompt = input(":")
#     try:
#         exec("%s = " + import_module(prompt) % prompt)
...
def main_model(n, model_int, dec, functions, save_file_name, data, password, email):
    ...
    logging.info(text)
    p("""Functions(funcs), Reboot(rebo), Write, Read, Change Password(cp), Help, Change email(ce) or Quit?""")
    sleep(.300)
    command = input(":")
    if command == "funcs":
        text = ('{} {} {}: User chosed functions. Sending the user to specify what function.'.format(datetime.datetime.hour, datetime.datetime.minute, datetime.datetime.second))
        logging.info(text)
...
def app(o_system, n, model_int, dec, functions, save_file_name, data, password, email):
    if o_system == "Darwin":
        ...
        if prompt == "y":
            ...
            ask(n, model_int, dec, functions, save_file_name, password, data, email)
...
def ask(n, model_int, dec, functions, save_file_name, data, password, email):
    text = ('{} {} {}: Informing user of how to use function selector'.format(datetime.datetime.hour, datetime.datetime.minute, datetime.datetime.second))
    logging.info(text)
...
""")
        ...
        text = ('{} {} {}: The user chose the search function. Sending the user to the search engines room.'.format(datetime.datetime.hour, datetime.datetime.minute, datetime.datetime.second))
        logging.info(text)
        search_engines(n, model_int, dec, functions, save_file_name, data, password, email)
    elif function == "email":
        text = ('{} {} {}: The user chose the email function. Sending the user to email sender.'.format(datetime.datetime.hour, datetime.datetime.minute, datetime.datetime.second))
        logging.info(text)
        email_function(n, model_int, dec, functions, save_file_name, data, password, email)
    elif function == "app":
        text = ('{} {} {}: The user chose app launcher. Sending the user to app launcher function.'.format(datetime.datetime.hour, datetime.datetime.minute, datetime.datetime.second))
        logging.info(text)
...
def sir_processor(model_int, dec, functions, save_file_name, data, password, email):
    ...
    if username == '':
        ...
        logging.info(text)
        dump_username(username, model_int, dec, functions, save_file_name, data, password, email)
