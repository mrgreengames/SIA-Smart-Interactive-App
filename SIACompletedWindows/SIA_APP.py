...
class MissingFilesProtocol(Error):
    """Raised when SIA can't find its files"""
    pass


class EmergencyProtocol(Error):
    """Raised when an Emergency Protocol has been called"""
    pass


class IncorrectPasswordProtocol(Error):
    """Raised when the incorrect password has been entered"""
    pass


class MissingImportsProtocol(Error):
    """Raised when SIA can't import the modules needed"""
    pass


class WrongPyInterpreterProtocol(Error):
    """Raised when SIA is being ran on a Python Interpreter that is not 3.6"""
    pass


class ExtensionNotFoundProtocol(Error):
    """Raised when a given extension cannot be found or is not compatible with SIA"""
    pass


class IncorrectEmailProtocol(Error):
    """Raised when the incorrect email has been input"""
    pass


class ExtensionNotRealProtocol(Error):
    """Raised when a given extension is not real"""
    pass


class UsedKeyProtocol(Error):
    """Raised when a given extension is using the same key as another extension installed"""
    pass
...
def protocol_check(protocol, reason=None):
    if protocol == 1:
        ...
        raise MissingFilesProtocol("File not found")
    if protocol == 2:
        print("EMERGENCY PROTOCOL HAS BEEN CALLED")
        if reason is None:
            print("REASON HAS NOT BEEN STATED")
            raise EmergencyProtocol("Emergency protocol has been called with no reason.")
        else:
            print("REASON FOR THIS IS %s" % reason)
            raise EmergencyProtocol("Emergency protocol has been called; the reason being {}".format(reason))
    if protocol == 3:
        ...
        if prompt == overidepasswor:
            ...
            with open(decrypt_msg("cfib", DECRYPTION_DICT) + '2.pkl') as file:
                pickle.dump("true", file)
            protocol = 0
            raise IncorrectPasswordProtocol('Incorrect password has been entered, locked out user')
    if protocol == 4:
        print("MISSING IMPORTS")
        print("PLEASE RUN SIA ON PYTHON 3.6")
        raise MissingImportsProtocol("Please run SIA on Python 3.6")
    if protocol == 5:
        print("PLEASE RUN SIA ON PYTHON 3.6")
        raise WrongPyInterpreterProtocol("Please do run SIA on Python 3.6")
    if protocol == 6:
        print("EXTENSION CANNOT BE FOUND OR IS NOT COMPATIBLE WITH SIA")
        raise ExtensionNotFoundProtocol("Extension cannot be found")
    if protocol == 7:
        print("INCORRECT EMAIL HAS BEEN ENTERED")
        raise IncorrectEmailProtocol("Incorrect email has been entered")
    if protocol == 8:
        print("EXTENSION NOT REAL")
        raise ExtensionNotRealProtocol("Extension is not real")
    if protocol == 9:
        print("EXTENSION IS USING ANOTHER'S KEY")
        raise UsedKeyProtocol("Extension is using a key for another extension")
...
def answer(function, m, n, model_int,  functions, save_file_name, data, password, email, extensions):
    if function == 'calc':
        if m == 'add':
            ...
            p('Tons to tonnes(20)[For you Americans: Tons to Metric Ton(nes)')
            p('Fahrenheit to Celsius(21)')
            prompt = input(">")
            sleep(1)
...
def doctor_smith_ask(extern_obs, n, model_int,  functions, save_file_name, data, password, email, extensions):
    sleep(.300)
    if extern_obs == 'calc':
        text = ('chose calculator')
        return text
    elif extern_obs == "ngg":
        text = ('chose number guessing game')
        return text
    elif extern_obs == "search":
        text = ('chose the search object')
        return text
    elif extern_obs == "email":
        text = ('chose the email object')
        return text
    elif extern_obs == "app":
        text = ('chose app launcher')
        return text
    elif extern_obs == "diction":
        text = ('chose the dictionary')
        return text
    elif extern_obs == "goback":
        text = ('wants to go back')
        return text
    else:
