...
def app(o_system, n, model_int, dec, functions, save_file_name, data, password):
    if o_system == "Darwin":
        p("Hello %s who is also a Mac user!" % n)
        sleep(1)
        p("I am Sappy.")
        sleep(1)
        ...
        if prompt == "y":
            ...
            app(o_system, n, model_int, dec, functions, save_file_name, data, password)
    elif o_system == "Windows":
        p("Sorry!")
        sleep(1)
        p("Windows has not been encoded yet!")
        sleep(1)
        ask(n, model_int, dec, functions, save_file_name, data, password)
