...
def answer(function, m, n, model_int, dec, functions, save_file_name, data, password):
    if function == 'calc':
        if m == 'add':
            ...
            if int_checker(per):
                if int(per) <= 100:
                    # print("The answer is %s" % con_per_fra(int(per)))
                    pass
                else:
                    print("That is not a percentage because it is higher than 100")
                    pass
...
def app(o_system, n, model_int, dec, functions, save_file_name, data, password):
    if o_system == "Darwin":
        ...
        if prompt == "y":
            ...
            ask(o_system, n, model_int, dec, functions, save_file_name, data)
...
def credit():
    ...
    sleep(1)
    print("Apple for making it much more easier to launch apps via Python")
    sleep(1)
